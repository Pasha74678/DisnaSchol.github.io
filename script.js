...
function initProfilePage() {
    ...
    function loadProfile() {
        try {
            ...
            if (savedProfile) {
                ...
                // Создаем новый профиль с UID по умолчанию
                const defaultUid = generateUID();
                uidInput.textContent = defaultUid;
                showNoAvatar();
            }
        } catch (error) {
    ...
    function saveProfile() {
        try {
            ...
            if (existingProfile) {
                ...
                // Если профиль новый, генерируем новый UID
                uid = generateUID();
                uidInput.textContent = uid; // Устанавливаем сгенерированный UID
            }
    ...
    function clearProfile() {
        if (confirm('Вы уверены, что хотите очистить профиль? Это действие нельзя отменить.')) {
            ...
            uidInput.textContent = generateUID();
